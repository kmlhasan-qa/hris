...
test('TC-001 Get countries with valid token returns 200', async ({ request }) => {
...
test('TC-002 Get countries with invalid token returns 401', async ({ request }) => {
  const response = await request.get(`${BASE_URL}${ENDPOINT}`, {
    headers: {
  ...
  console.log(
    'get countries invalid token response:',
    JSON.stringify(body, null, 2)
  );
