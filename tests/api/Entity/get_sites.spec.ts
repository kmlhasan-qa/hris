...
test('TC-001 Get sites with valid token returns 200', async ({ request }) => {
...
test('TC-002 Get sites with invalid token returns 401', async ({ request }) => {
  const response = await request.get(`${BASE_URL}${ENDPOINT}`, {
    headers: {
  ...
  console.log(
    'get sites invalid token response:',
    JSON.stringify(body, null, 2)
  );
