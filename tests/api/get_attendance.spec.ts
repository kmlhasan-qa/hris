...
async function assertApiResponse(response: APIResponse) {
  ...
  test('TC-001 Get attendance with valid token returns 200 success', async ({ request }) => {
    const token = getAuthToken();
    test.skip(!token, 'Auth token not found; run tests/api/hris_login.spec.ts first to generate finalToken');

    const response = await request.get(`${BASE_URL}/api/attendances`, {
      headers: {
  ...
  test('TC-002 Get attendance with invalid token returns 401 error', async ({ request }) => {
    const response = await request.get(`${BASE_URL}/api/attendances`, {
      headers: {
