...
async function assertApiResponse(response: APIResponse) {
  ...
  test('TC-001 Get attendance recap with valid token returns 200 success', async ({ request }) => {
    const token = getAuthToken();
    test.skip(!token, 'Auth token not found; run tests/api/hris_login.spec.ts first to generate finalToken');

    const response = await request.get(`${BASE_URL}/api/attendances/recap`, {
      headers: {
  ...
  test('TC-002 Get attendance recap with invalid token returns 401 error', async ({ request }) => {
    const response = await request.get(`${BASE_URL}/api/attendances/recap`, {
      headers: {
